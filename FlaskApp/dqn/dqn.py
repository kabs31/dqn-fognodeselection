...
results_folder = "C:/Users/91638/Desktop/DA/flask/FlaskApp/results/recent-model"
env = fenv.env
train = False
render = False
...
def traindqn():
    progress = []
    episode_length = []
    action_frequency = [0] * env.action_space.n
    loss_history = []
    """
    ...
    for ep_cnt in range(num_train_eps):
        ...
        while not done:
            ...
            loss = dqn_agent.learn(batchsize=batchsize)
        ...
        episode_length.append(episode_step_cnt)
        ...
        if ep_cnt % 10 == 0:
            progress.append((ep_cnt, current_avg_score, dqn_agent.epsilon))

        if current_avg_score >= best_score:
            dqn_agent.save_model('{}/dqn_model'.format(results_folder))
            best_score = current_avg_score
    
    with open('{}/loss_history.pkl'.format(results_folder), 'wb') as f:
        pickle.dump(loss_history, f)

    with open('{}/action_frequency.pkl'.format(results_folder), 'wb') as f:
            pickle.dump(action_frequency, f)

    with open('{}/ep_length.pkl'.format(results_folder), 'wb') as f:
        pickle.dump(episode_length, f)

    with open('{}/progress.pkl'.format(results_folder), 'wb') as f:
        pickle.dump(progress, f)

    with open('{}/train_reward_history.pkl'.format(results_folder), 'wb') as f:
        pickle.dump(reward_history, f)

    with open('{}/train_epsilon_history.pkl'.format(results_folder), 'wb') as f:
...
if __name__ ==  '__main__':
    ...
    if args.results_folder is None:
            args.results_folder = "C:/Users/91638/Desktop/DA/flask/FlaskApp/results/recent-model"
