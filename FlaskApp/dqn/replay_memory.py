...
class ReplayMemory:
    ...
    def sample(self, batch_size, device):
        ...
        """
        self.buffer_state.pop()
        indices_to_sample = random.sample(range(len(self.buffer_state)), batch_size)
        states = torch.as_tensor(np.array(self.buffer_state)[indices_to_sample]).float().to(device)
        actions = torch.as_tensor(np.array(self.buffer_action)[indices_to_sample]).to(device)
        next_states = torch.as_tensor(np.array(self.buffer_next_state)[indices_to_sample]).float().to(device)
        rewards = torch.as_tensor(np.array(self.buffer_reward)[indices_to_sample]).float().to(device)
        dones = torch.as_tensor(np.array(self.buffer_done)[indices_to_sample]).to(device)
